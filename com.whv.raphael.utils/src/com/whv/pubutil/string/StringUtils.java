package com.whv.pubutil.string;
...
public class StringUtils
{
    ...
    /**
	 * @说明 返回输入字符串的缩写,如果字符串为空或NULL返回空,<br />
	 * 如果长度小于指定的宽度返回原串
	 * @param input 输入的字符串
	 * @param width 保留的字符串长度
	 * @return 返回结果如test....
	 */
	public static String abbreviate2(String input,int width){
		if(input == null)
			return "";
		else if(input.trim().equals(""))
			return "";
		
		if(input.trim().length() <= width)
			return input.trim();
		
		String abbreviate = input.trim().subSequence(0, width)+"...";
		return abbreviate;
	}
...
label0:
    ...
    public static String insertAtEveryLength(String str,int len, String split){
